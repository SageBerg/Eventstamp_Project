...
#######functions are written in the order they should be called within eventstamp.py's main function
...
def draw_time_scales():
    ...
    minute_scale = Scale(root, from_=0, to=59, orient=HORIZONTAL, length=221)
    ...
    hour_scale = Scale(root, from_=0, to=23, orient=HORIZONTAL, length=221)
    ...
    month_scale = Scale(root, from_=1, to=12, orient=HORIZONTAL, length=221)
    ...
    year_scale = Scale(root, from_=2014, to=2100, orient=HORIZONTAL, length=221)
    year_scale.grid(row=11, column=14, columnspan=2, padx=5) 

    refresh_scales(minute_scale, hour_scale, day_scale, month_scale, year_scale)

    refresh_scales_button = Button(root, height=1, width=6, text="Current", \
    command=lambda 
    minute = minute_scale,
    hour   = hour_scale,
    day    = day_scale,
    ...
    set_scales_to_last_time_button = Button(root, height=1, width=6, text="Last Time", \
    command=lambda
    minute = minute_scale,
    hour   = hour_scale,
    day    = day_scale,
    month  = month_scale,
    year   = year_scale:
    set_scales_to_last_eventstamp(minute,hour,day,month,year), wraplength=100)
...
def draw_activity_buttons(event_list, happiness, note_entry_box, stress_bool, scales_bool, scales_list, display, display_list):
    buttons_list = list() 
    i = 0
    j = 0
    for event in event_list:
        buttons_list.append(Button(root, text=event[0].title(), width = 6, height = 4, 
        ...
        bg=event[1], fg=event[2], activebackground=event[1], activeforeground = event[2], wraplength=70)) 
        buttons_list[-1].grid(row=j, column=i*2, columnspan=2, padx=5, pady=5)
        j += 1
        j = j%7
        if j == 0:
            i += 1
...
def draw_people_checkboxes(people_list):
    ...
    for person in people_list:
        check_box_list.append(Checkbutton(root, text=person[0].title(), variable=person[1], \
        width=6, bg='white', activebackground='white'))
        check_box_list[-1].grid(row=r, column=k*2, columnspan=2, padx=5, pady=5)
        k += 1
        k = k%4
        if k == 0:
            r += 1
...
def draw_happiness_buttons():
    ...
    for i in range(1,6):
        ...
        root, text=str(i), variable=happiness, value=i, highlightcolor='white', \
        indicatoron=0, height=2, width=15, bg=happiness_color_dict[str(i)][0], \
        activebackground=happiness_color_dict[str(i)][0])) #, selectcolor=happiness_color_dict[str(i)][0]))
        radio_button_list[-1].grid(row=10, column=7+i)
    radio_button_list[2].select() #has happiness level 3 selected by default
    return happiness
...
def draw_note_buttons(note_list, note_entry_box, note_entry_string):
    ...
    for note in note_list:
        note_button_list.append(Button(root, text=note, width=12, height=4, bg='white', wraplength=100,
        command=lambda button_label=note, change_note=note_entry_string.set: change_note(button_label)))
...
def draw_note_entry_box():
    ...
    note_entry_box = Entry(root, textvariable=note_entry_string, width=25, justify=CENTER)
...
def draw_stress_box(stress_bool):
    stress_box = Checkbutton(root, text='Stressed', variable=stress_bool, width=12) #, \
...
def draw_time_scales_check_box(scales_bool):
    check_box = Checkbutton(root, text='Scales', width=6, variable=scales_bool)
...
def draw_delete_last_stamp_button(display, display_list):
    ...
    delete_last_stamp_button.grid(row=9, column=12, columnspan=1, padx=5, pady=5)

def draw_last_stamp_button(last_stamp_string):
    calendar_button = Button(root, text='See Last Stamp', \
    command=lambda change_text=last_stamp_string.set: change_text(get_last_stamp()), width=12)
    calendar_button.grid(row=9, column=8, padx=5, pady=5)

def draw_last_stamp_entry():
    last_stamp_string = StringVar()
    last_stamp_entry = Entry(root, textvariable=last_stamp_string, width=48, justify=CENTER)
...
def draw_calendar_buttons():
    ...
    callback_list = [eventstamp_calendar, happiness_calendar, people_calendar, stress_calendar]
    for i in range(4):
        calendar_button = Button(root, text=button_text_list[i], width=12, \
...
def draw_realtime_eventstamp_display():
    ...
    for i in range(len(eventstamp_list)):
        ...
        (datetime.today().year -2000, datetime.today().month, datetime.today().day):
            color = 'black'
            ...
            if eventstamp_list[i-1].day == eventstamp_list[i].day:
                start_x = eventstamp_list[i-1].minute + eventstamp_list[i-1].hour*60
            else:
                start_x = 0
