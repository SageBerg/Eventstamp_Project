...
class Eventstamp_Calendar(object): #needs a clean up
    ...
    def __init__(self, window_name):
        ...
        self.date_dict = eventstamp_parser.make_recent_date_dict()
        inverted_date_dict = {date:number for number, date in self.date_dict.items()}
        temp = list()
        ...
        self.width = min(99 + len(self.date_dict)*200 + 100, 1599)  #width of calendar canvas 
        ...
        self.fill_calendar_with_titles() #labels each block in the calendar

        self.root.mainloop()

    def draw_blank_calendar(self):
        for hour in range(24): #create the titles and lines for each hour
            self.calendar.create_line(0, hour*60 + 20, self.width, hour*60 + 20) #+20 px to fit titles at top
            ...
            if hour < 12:
                ...
                hour_title += ' PM'
            self.calendar.create_text((50, hour*60 +30), text=hour_title) #place the titles of each hour
            self.calendar.create_text((self.width -50, hour*60 +30), text=hour_title) 
        self.calendar.create_line(0, 1460, self.width, 1460) #create last line in calendar
        if len(self.date_dict) <  7:
            for i in range(len(self.date_dict)):
                self.calendar.create_text(200 + i*200, 10, text=self.prettyify_date(self.date_dict[i]))
                self.calendar.create_text(200 + i*200, 1470, text=self.prettyify_date(self.date_dict[i]))
        else:
            index = len(self.date_dict) -1
            for i in range(0,7):
                self.calendar.create_text(1400 - i*200, 10, text=self.prettyify_date(self.date_dict[index]))
                self.calendar.create_text(1400 - i*200, 1470, text=self.prettyify_date(self.date_dict[index]))
                index -= 1
        ...
        for i in range(len(self.eventstamp_list)):
            if self.eventstamp_list[i-1].date in inverted_date_dict and \
               self.eventstamp_list[i].date in inverted_date_dict:
                ...
                block_color = eventstamp_variables.color_dict[curr_event.what.strip()][0] 
                
                topx    = 100 + inverted_date_dict[prev_event.date]*200  
                bottomx = 300 + inverted_date_dict[curr_event.date]*200 
                topy    = 20  + prev_event.hour*60 + prev_event.minute #y is in minutes
                bottomy = 20  + curr_event.hour*60 + curr_event.minute #draws backwards from stamp time
               
                if prev_event.hour == 23 and prev_event.minute == 60:
                    topy = 20
                    topx += 200
                    
                self.calendar.create_rectangle(topx, topy, bottomx, bottomy, fill=block_color, width=0) 
        ...
        for i in range(1,len(self.eventstamp_list)):
            ...
            if self.eventstamp_list[i-1].date in inverted_date_dict and \
               ...
               self.eventstamp_list[i].what.strip() != 'Sexual' and duration >= 15:
                ...
                text_color = eventstamp_variables.color_dict[curr_event.what.strip()][1]
                ...
                x = 200 + 200*inverted_date_dict[self.eventstamp_list[i].date]
                y = 26  + 60*prev_event.hour + prev_event.minute + y_change

                self.calendar.create_text((x,y), text=block_title, fill=text_color)
        
                if curr_event.who.strip() != '' and duration >= 30: #show people
                    number_of_people = min(3,len(curr_event.who.strip().split()))
                    people_string = 'with '
                    ...
                    for i in range(number_of_people):
                        people_string += curr_event.who.strip().split()[i] + separator_list[i] 
                    self.calendar.create_text((x,y + 16), text=people_string, fill=text_color) 
        ...
        for i in range(1,len(self.eventstamp_list)):
            if self.eventstamp_list[i-1].date in inverted_date_dict and \
               self.eventstamp_list[i].date in inverted_date_dict:
                ...
                block_color = eventstamp_variables.happiness_color_dict[curr_event.happiness.strip()][0]
        ...
        for i in range(1,len(self.eventstamp_list)):
            ...
            if self.eventstamp_list[i-1].date in inverted_date_dict and \
               ...
               self.eventstamp_list[i].what.strip() != 'Sexual' and duration >= 15:
                ...
                text_color = eventstamp_variables.happiness_color_dict[curr_event.happiness.strip()][1]
        ...
        for i in range(1,len(self.eventstamp_list)):
            if self.eventstamp_list[i-1].date in inverted_date_dict and \
               self.eventstamp_list[i].date in inverted_date_dict: 
                ...
                block_color = eventstamp_variables.stress_color_dict[curr_event.stress][0] 
        ...
        for i in range(1,len(self.eventstamp_list)):
            ...
            if self.eventstamp_list[i-1].date in inverted_date_dict and \
               ...
               self.eventstamp_list[i].what.strip() != 'Sexual' and duration >= 15:
                ...
                text_color = eventstamp_variables.stress_color_dict[curr_event.stress][1]
